function dydt = clark_rhs(t, y, lag, pars)
 % % Input
 % 	- time
 % 	- current solutions
 % 	- lag solution vector
 ...
 % 	- column vector of equation values for use with solver
    ...
    % Get ovarian hormones
        [E2,P4,P4del,~,IHdel] = clark_OvarianHormones(y,lag,pars);

	% Synthesis and release of LH
        syn_LH=(v_0LH + v_1LH * Hill_plus(E2,Km_LH,8)) / (1+P4del/Ki_LH_P);
	...
	% Ovarian Stages
		d_RcF = b*FSH + (c_1*FSH-c_2*LH^alpha)*RcF;
        d_SeF = c_2*LH^alpha*RcF + (c_3*LH^beta-c_4*LH)*SeF;
        d_PrF = c_4*LH*SeF - c_5*LH^gamma*PrF;
		d_Sc1 = c_5*((LH)^gamma)*PrF-d_1*Sc1;
	...
	% Assign values for return
		dydt = [d_RP_LH d_LH d_RP_FSH d_FSH d_RcF d_SeF d_PrF d_Sc1 d_Sc2 d_Lut1 d_Lut2 d_Lut3 d_Lut4]';
