function [E2,P4,IH,IHdel] = margolskee_OvarianHormones(x,lag_sol,pars)
    ...
    % get current state variable values
		...
		RP_FSH = x(3);
		FSH    = x(4);
		ReF    = x(5); %current ReF level
		SeF    = x(6); %current SeF level
		PrF    = x(7); %current PrF level
		Ov1    = x(8); %current Ov1 level
		Ov2    = x(9); %current Ov2 level
